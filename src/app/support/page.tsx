...
type Bank = { id: string; bankName: string; last4: string; holder: string };

export default function WithdrawPage() {
  const [balance, setBalance] = useState<number>(0);
  const [minWithdraw, setMinWithdraw] = useState<number>(20000);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const selected =
          banks.accounts.find((a: Bank) => a.id === banks.selectedId) || banks.accounts[0] || null;
  ...
  const quick = (amt: number) => setAmount(amt);
    ...
    <>
      ...
      <PageContainer className="space-y-md">
        <Card>
          ...
          <Link href="/banks" className="block rounded-[12px] border border-border p-md bg-white mt-md hover:shadow-sm transition">
            {bank ? (
              <>
          ...
          <div className="flex gap-sm mb-md mt-md">
            {[50000, 100000, 200000].map((amt) => (
              <button key={amt} className="flex-1 px-md py-sm rounded-control border border-border text-body" onClick={() => quick(amt)}>
