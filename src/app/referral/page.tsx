...
export default function ReferralPage() {
  const [tab, setTab] = useState('doing'); // doing | pending | done
        ...
        <div className="rounded-[12px] bg-[color:#F6F1E8] p-[6px] flex gap-sm" role="tablist" aria-label="Lọc trạng thái lời mời">
          {[
            ...
            <button
              ...
              onClick={() => setTab(t.id)}
              className={`flex-1 py-xs rounded-full ${tab === t.id ? 'bg-white shadow-sm text-body font-medium' : 'text-caption text-text-muted'}`}
            >
              {t.label}
            </button>
          ))}
