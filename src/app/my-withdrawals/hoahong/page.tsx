...
export default function HoaHongPage({ embedded = false }) {
  const [rows, setRows] = useState([]);

  useEffect(() => {
    api.history.commissions.get().then((res) => setRows(res.items));
  }, []);

  const todayTotal = useMemo(() => {
    const today = new Date().toDateString();
    return rows.filter(r => new Date(r.createdAt).toDateString() === today)
    ...
    <>
      ...
      <PageContainer className={embedded ? "" : "space-y-md"}>
        <div className="flex justify-between text-body mb-md">
          <div>
            Hôm nay <span style={{ color: "#2E7D32" }}>
              +{todayTotal.toLocaleString("vi-VN")}đ
            </span>
          </div>
          <div>
            Tháng này <span style={{ color: "#2E7D32" }}>
        ...
        <div className="space-y-sm">
          ...
          {rows.length === 0 && (
            <div className="text-caption text-text-muted">Chưa có hoa hồng.</div>
          )}
        </div>
      </PageContainer>
    </>
